...
const BookingPage = () => {
    ...
    const fetchRoomData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5001/room-details/${roomID}`
        );
  ...
  const handleConfirmBooking = () => {
    ...
    axios
      .post(`http://localhost:5001/book-room/${roomID}`, bookingData)
      .then((response) => {
        console.log("Booking confirmed:", response.data);
        axios
          .post(
            `http://localhost:5001/create-transaction/${roomID}`,
            TransactionData
          )
      ...
      <div style={{ width: "70vw", marginLeft: "15vw" }}>
        ...
        <div style={{ display: "flex" }}>
          <div style={{ flex: 1, marginRight: "20px", marginTop: "14vh" }}>
            <h4>
              {" "}
              <b>Hotel Name : {roomData.title}</b>{" "}
            </h4>
            <hr />
            <h5 style={{ fontWeight: "600" }}>Your Holidays</h5>
            <p>
              From {selectedCheckInDate.toDateString()} to{" "}
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              ...
              <h6>
                {" "}
                <b>Price/Night -</b> Rs. {roomPrice}{" "}
              </h6>
              <h6>
                {" "}
                <b>Total Stay &nbsp; -</b> {calculateDays()} Days{" "}
              </h6>
              <h6>
                {" "}
                <b>Total Cost &nbsp; -</b> Rs. {calculateTotalPrice()}{" "}
              </h6>
              <hr></hr>
              <h6>
                {" "}
                <b>Service Charge &nbsp;-</b> Rs. {ServiceCharge}
              </h6>
              <h6>
                <b>Addons Price-</b> Rs {Addons}
              </h6>
              <h6>
                {" "}
                <b>Payable Amount -</b> Rs.{" "}
                {calculateTotalPrice() + ServiceCharge + Addons}
              </h6>
              <p style={{ fontSize: "10px", color: "#FF5252" }}>
                Taxes may apply at payment
              </p>
              <hr></hr>
              ...
              {isEmailValid ? (
                <button
                  ...
                  style={{ width: "100%", border: "0px", padding: "5px" }}
                >
                  Confirm Booking
                </button>
              ) : (
